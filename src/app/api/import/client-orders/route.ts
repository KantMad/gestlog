...
export async function POST(request: NextRequest) {
  try {
    ...
    let result;
    if (detectMcsFormat(buffer) === "client-order") {
      // Format MCS (StatGen commande client) : parsing dédié, sans mapping de colonnes.
      result = await importMcsClientOrders(buffer, seasonId);
    } else {
      ...
      result = await importClientOrders(sheets[0], JSON.parse(mappingJson), seasonId);
    }

    await prisma.importLog.create({
      data: {
        seasonId,
        importType: "CLIENT_ORDER",
        fileName: file.name,
