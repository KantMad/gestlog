...
import { importClientOrders } from "@/lib/import/client-order-mapper";
import { detectMcsFormat } from "@/lib/import/mcs-format";
import { importMcsClientOrders } from "@/lib/import/mcs-mapper";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const isMcs = detectMcsFormat(buffer) === "client-order";
    ...
    // Log créé d'abord → son id tague les entités créées (permet de supprimer l'import).
    const log = await prisma.importLog.create({
      data: { seasonId, importType: "CLIENT_ORDER", fileName: file.name, rowCount: 0 },
    });

    let result;
    try {
      result = isMcs
        ? await importMcsClientOrders(buffer, seasonId, log.id)
        : await importClientOrders(sheet!, JSON.parse(mappingJson!), seasonId, log.id);
    } catch (e) {
    ...
    return handleApiError(e, "api/import/client-orders");
  }
}
